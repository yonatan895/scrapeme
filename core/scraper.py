...
from config.models import SiteConfig, StepBlock, FieldConfig
from core.waits import Waiter
from core.frames import FramesNavigator
from core.retry import selenium_retry
from core.exceptions import ExtractionError, ErrorContext
        ...
        *,
        ...
        if artifact_dir:
            ...
            self._capture = ArtifactCapture(
                waiter.driver, Path(), logger, enabled=False
            )
                    ...
                    try:
                        ...
                        if self._capture.enabled:
                            self._capture.capture(
                                f"{self._config.name}_{step.name}_{field.name}"
                            )
